...
function buildSuggestionPrompt(
  ...
  return [
    ...
    'Precomputed structured evidence items in the local context are AUTHORITATIVE backend aggregates, not claims to be independently re-verified. Trust them as primary verification for the claim they describe. Examples of authoritative items and what they verify:',
    '  - "Wikipedia citations rank: #N of M" — authoritative rank claim.',
    '  - "Wikipedia <metric> comparison: <self>=X, <competitor>=Y, ..." — authoritative per-company values for that metric.',
    '  - "Wikipedia <metric> lead above average: X" — authoritative delta.',
    '  - "Wikipedia competitors with <feature>: N of M (X%)" — authoritative prevalence count.',
    '  - "Wikipedia infobox fields: field1, field2, ..." — authoritative list of fields present on the article.',
    '  - "Wikipedia last edited: <iso-date>" / "Wikipedia edits in last 30 days: N" — authoritative recency metrics.',
    '  - "Wikipedia has <feature>: true|false" — authoritative presence flag.',
    'When a suggestion\'s quantitative claim matches an authoritative item within rounding tolerance (±0.1 for percentages, exact for counts and ranks), mark it Correct with HIGH confidence. Do NOT downgrade to "Needs Review" solely because the fetched competitor or target pages do not independently re-expose these aggregates — the aggregates are already computed from the backend dataset. Only downgrade when the authoritative item contradicts the suggestion or is absent for the specific claim being made.',
