...
function buildSuggestionPrompt(
  ...
  return [
    ...
    'Tolerance bands (treat differences within these as acceptable, not errors):',
    '  - Percentages and shares: ±2 percentage points (or ±0.5pp if a precise decimal is quoted).',
    '  - Small counts (sections, images, categories, infobox fields): ±2.',
    '  - Larger counts (citations, references, word counts): ±10% or ±5 absolute, whichever is larger.',
    ...
    'When a live fetched page contradicts a backend aggregate BEYOND these tolerances (e.g., backend says 6 images but the live page shows 12), mark Incorrect and return a correctedSuggestion using the live value. When the delta is WITHIN tolerance, mark Correct.',
    ...
    'Wikipedia-specific notes: (a) Section counts differ depending on whether trailing sections (References, External links, See Also) are included — a delta of ±2 is not by itself an error. (b) Infobox field lists from the backend are flattened from the article\'s infobox template; when the live page clearly surfaces the same fields (by label or by equivalent prose), count that as verified.',
